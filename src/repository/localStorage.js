const setValuesLocalStorage = ({value, name}, {itemKey}) => {
    ...
    const getDefaultValueLocalStorage = (key, itemKey) => {
      const value = localStorage.getItem(`${itemKey}`) && JSON.parse(localStorage.getItem(`${itemKey}`))[`${key}`] || null
      return value
    }

    const getInitialValue = (itemKey) =>  {
      const data=  localStorage.getItem(`${itemKey}`) && JSON.parse(localStorage.getItem(`${itemKey}`)) || {}
      return data
    }
